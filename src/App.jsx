...
export default function App() {
  ...
  const todayKey =
    weekDays.find((d) => d.isToday)?.key ?? weekDays[0].key
  ...
  const isCurrentWeek = (() => {
    ...
    const selectedMonday = new Date(weekDays[0].iso)
    selectedMonday.setHours(0, 0, 0, 0)

    return (
      currentMonday.getTime() === selectedMonday.getTime()
    )
  })()
 ...
 useEffect(() => {
  ...
  pastOrToday.forEach((d) => {
    const tasks = SCHEDULE[d.key].filter(
      (t) => t.tag !== 'Bonus'
    )
    ...
    const criticalTasks = SCHEDULE[d.key].filter(
      (t) => t.critical
    )
